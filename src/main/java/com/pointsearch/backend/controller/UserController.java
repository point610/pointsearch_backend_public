...
import com.pointsearch.backend.config.WxOpenConfig;
...
import com.pointsearch.backend.model.dto.user.*;
...
public class UserController {
    ...
    public BaseResponse<Boolean> addUser(@RequestBody UserAddRequest userAddRequest, HttpServletRequest request) {
        ...
        boolean result = userService.addUser(userAddRequest.getUserName(), userAddRequest.getUserAccount(),
                userAddRequest.getUserRole(), userAddRequest.getUserAvatar());
     ...
     * @Author point
     * @Date 20:43 2023/7/24
     **/
    @PostMapping("/list/delete")
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<Boolean> deleteUsers(@RequestBody List<DeleteRequest> deleteRequest, HttpServletRequest request) {
        if (deleteRequest == null || deleteRequest.size() <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        boolean result = userService.removeBatchByIds(deleteRequest);
        return ResultUtils.success(result);
    }
    ...
    public BaseResponse<Boolean> updateMyPassword(@RequestBody UserUpdatePasswordRequest userUpdatePasswordRequest, HttpServletRequest request) {
        ...
        userService.updatePassword(userUpdatePasswordRequest.getOldPassword(),
                userUpdatePasswordRequest.getNewPassword(), userUpdatePasswordRequest.getCheckPassword(), request);
     ...
     *
     * @param request
     * @return com.pointapi.backend.common.BaseResponse<com.pointapi.backend.model.vo.ASKVO>
     * @Author point
     * @Date 18:29 2023/7/26
     **/
    @PostMapping("/change/ask")
    public BaseResponse<ASKVO> changeASKey(HttpServletRequest request) {
        ASKVO askvo = userService.changeASK(request);

        return ResultUtils.success(askvo);
    }
